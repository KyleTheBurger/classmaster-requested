			...
			<v-row>
				...
				<v-col cols="6">
					...
					<p class="displaySelectedSection">
						<span class="strand"
							><strong>STRAND:</strong> {{ strand }}</span
						>
						<span class="section"
							><strong>SECTION:</strong> {{ section }}</span
						>
					</p>
					...
					<v-btn
						...
						class="setSection"
						:disabled="isSetSectionButtonDisabled"
						@click="setSection"
						><span class="material-symbols-outlined">
...
export default {
	...
	methods: {
		async getStudents() {
			try {
				const response = await axios.get(
					"http://localhost:3000/sectioning/src/database/Enrollmentdb.php"
				);
		...
		async getSections() {
			try {
				const response = await axios.get(
					"http://localhost:3000/sectioning/src/database/SectionFilter.php"
				);
		...
		async setSection() {
			try {
				const response = await axios.post(
					"http://localhost:3000/sectioning/src/database/SetSection.php",
					{
						section: this.section,
						students: this.sectionedStudents.map(
		...
		updateGenderCounts() {
			this.genderCounts.male = this.sectionedStudents.filter(
				(student) => student.gender === "Male"
			).length;
			this.genderCounts.female = this.sectionedStudents.filter(
